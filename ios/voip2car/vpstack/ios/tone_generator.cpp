...
Tones::Tones(unsigned AVolume, unsigned ASampleRate): m_sampleRate(ASampleRate), m_masterVolume(AVolume)
{
	Construct();
}


void Tones::Construct()
{
	m_lastOperation = 0;
	m_lastFrequency1 = 0;
	m_lastFrequency2 = 0;
...
bool Tones::Generate(char AOperation, unsigned AFrequency1, unsigned AFrequency2, unsigned ADutartionMilliseconds, unsigned AVolume)
{
	if (m_lastOperation  != AOperation  ||
		m_lastFrequency1 != AFrequency1 ||
		m_lastFrequency2 != AFrequency2) {
		m_lastOperation  = AOperation;
		m_lastFrequency1 = AFrequency1;
		m_lastFrequency2 = AFrequency2;
	...
	switch (AOperation) {
		case '+':
			return Juxtapose(AFrequency1, AFrequency2, ADutartionMilliseconds, AVolume);
			
		case 'x':
			return Modulate(AFrequency1, AFrequency2, ADutartionMilliseconds, AVolume);
			
		case '-':
			return PureTone(AFrequency1, ADutartionMilliseconds, AVolume);
			
		case ' ':
			return Silence(ADutartionMilliseconds);
	}
...
bool Tones::Juxtapose(unsigned AFrequency1, unsigned AFrequency2, unsigned ADurationMilliseconds, unsigned AVolume)
{
	if (AFrequency1 < MinFrequency || AFrequency1 > m_maxFrequency ||
		AFrequency2 < MinFrequency || AFrequency2 > m_maxFrequency)
		return false;
	
	// TODO this gived 8000 samples for 100 ms !!!
	//unsigned samples = CalcSamples(milliseconds, AFrequency1, AFrequency2);
	unsigned samples = ADurationMilliseconds * m_sampleRate / 1000;
	while (samples-- > 0) {
		int a1 = sine(m_angle1, m_sampleRate);
		int a2 = sine(m_angle2, m_sampleRate);
		
		AddSample((a1 + a2) / 2, AVolume);
		
		m_angle1 += AFrequency1;
		if (m_angle1 >= (int)m_sampleRate) 
			m_angle1 -= m_sampleRate;
		
		m_angle2 += AFrequency2;
...
bool Tones::Modulate(unsigned AFrequency1, unsigned modulator, unsigned milliseconds, unsigned volume)
{
	if (AFrequency1 > m_maxFrequency || AFrequency1 > m_maxFrequency || modulator < MinModulation || modulator >= AFrequency1/2)
		return false;
	
	unsigned samples = CalcSamples(milliseconds, AFrequency1, modulator);
...
bool Tones::PureTone(unsigned AFrequency1, unsigned milliseconds, unsigned volume)
{
	if (AFrequency1 == 2100) {
	...
	unsigned samples = CalcSamples(milliseconds, AFrequency1);
	while (samples-- > 0) {
...
bool Tones::Silence(unsigned milliseconds)
{
...
unsigned Tones::CalcSamples(unsigned ms, unsigned f1, unsigned f2)
{
...
void Tones::AddSample(int sample, unsigned volume)
{
