...
{
#if defined(RGB24_INPUT) || defined (RGB16_INPUT) || defined (RGB12_INPUT) || defined(YUV420SEMIPLANAR_INPUT)
    ccRGBtoYUV = NULL;
#endif
//iEncoderControl
}
...
TAVCEI_RETVAL PVAVCEncoder::Init(TAVCEIInputFormat* aVidInFormat, TAVCEIEncodeParam* aEncParam, AVCEncParams& aEncOption)
{
    ...
    // allocate iYUVIn
    if ((iSrcWidth&0xF) || (iSrcHeight&0xF) || (iVideoFormat != EAVCEI_VDOFMT_YUV420 && iVideoFormat != EAVCEI_VDOFMT_YVU420)) /* Not multiple of 16 */
    {
    ...
    if (iVideoFormat == EAVCEI_VDOFMT_RGB16)
    {
#ifdef RGB16_INPUT
        ccRGBtoYUV = CCRGB16toYUV420::New();
#else
        return EAVCEI_FAIL;
#endif
    }
    ...
    if (iVideoFormat == EAVCEI_VDOFMT_UYVY)
    {
        ccRGBtoYUV = CCYUV422toYUV420::New();
    }


    if ((iVideoFormat == EAVCEI_VDOFMT_RGB24) || (iVideoFormat == EAVCEI_VDOFMT_RGB16) ||
		(iVideoFormat == EAVCEI_VDOFMT_RGB12) || (iVideoFormat == EAVCEI_VDOFMT_YUV420SEMIPLANAR) ||
		(iVideoFormat == EAVCEI_VDOFMT_UYVY))
    {
#if defined(RGB24_INPUT) || defined (RGB16_INPUT) || defined (RGB12_INPUT) || defined (YUV420SEMIPLANAR_INPUT)
                                                  ...
                                                  )
{
    ...
    if (iVideoFormat == EAVCEI_VDOFMT_YUV420 || iVideoFormat == EAVCEI_VDOFMT_YVU420)
#ifdef YUV_INPUT
    {
	...
	if(iVideoFormat == EAVCEI_VDOFMT_YVU420)
	{
		iVidIn.YCbCr[2] = (uint8*)(iVideoIn + iVidIn.height * iVidIn.pitch);
		iVidIn.YCbCr[1] = iVidIn.YCbCr[2] + ((iVidIn.height * iVidIn.pitch) >> 2);
	} else {
