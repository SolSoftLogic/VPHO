//
//  RegistrationCountryViewController.mm
//  Vphonet
//
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (indexPath.section == 0)
	{
		...
		VPCountry* country = [[VPCountry alloc] initFromDatabase:profile.country];
		...
		if (indexPath.row == 0) {
			...
			if (country.code != nil)
			{
				cell.textField.text = [NSString stringWithFormat:@"+%@ ",country.code];
		...
		UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"Done"];
		if (cell == nil) {
			cell = [[[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"Done"] autorelease];
			cell.selectionStyle = UITableViewCellSelectionStyleNone;
		}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (indexPath.section == 1) // Done
	{
		NSLog(@"userName: %@ password: %@ first: %@ last: %@ email: %@ country: %@ phone: %@",
			  profile.userName, profile.password, profile.firstName, 
			  profile.lastName, profile.email, profile.country, profile.phoneHome);
			  
		profile.state = nil;
		profile.city = nil;
		[self postRegistrationInfo:profile];
...
-(void)postRegistrationInfo:(VPProfile *)_info {
	...
	VPCountry *country = [[VPCountry alloc] initFromDatabase:_info.country];
	...
	body = [body stringByAppendingFormat:[NSString stringWithFormat:@"&frm_country=%@", [_info.countryCode stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]];
	...
	body = [body stringByAppendingFormat:@"&frm_phone=%@", [_info.phoneHome stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
	...
	_info.number = vp_number;
	NSLog(@"regcode: %@ vp_number: %@", regcode, vp_number);
	...
	[VPProfile insert:_info];

}

-(void)postConfirm:(VPProfile *)_info {
