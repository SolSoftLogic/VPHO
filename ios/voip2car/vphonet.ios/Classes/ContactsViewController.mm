...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
	...
	NSLog(@"Online: %d Offline: %d", [[[VPContactList sharedInstance] onlineContacts] count], [[[VPContactList sharedInstance] offlineContacts] count]);
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
	if ((section == 0) && [[[VPContactList sharedInstance] onlineContacts] count] == 0)
		 return 0;
...
//    }
	...
	if (contact) {
		...
		cell.imagePhoto.image = [UIImage imageWithContentsOfFile:[contact photoImageName]];

		
		cell.imageStatus.image = [UIImage imageNamed:[contact statusImageName]];
		
		if ([contact.firstName length] == 0) {
			cell.label.text = contact.userName;
		} else {
	...
	switch (typeSMS) {
		case SMSTYPE_CONTACTADDED:
		{
			UICustomAlertView *alert = [[UICustomAlertView alloc] 
										initWithTitle:[NSString stringWithFormat:NSLocalizedString(@"Request from %@", @""),[[notification userInfo] objectForKey:@"name"]]
			...
			[userInfo setObject:[[notification userInfo] objectForKey:@"name"] forKey:@"name"];
		...
		{
			VPContact* contact = [[VPContactList sharedInstance] contactByNumber:[[notification userInfo] objectForKey:@"number"]];
			if (contact) {
				contact.flags |= 0x04;
			}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
	if (alertView.tag == ALERT_CONTACTADDED) {
		...
		if (buttonIndex == 1) // accepted
		{
			...
			contact.userName = [userInfo objectForKey:@"name"];
