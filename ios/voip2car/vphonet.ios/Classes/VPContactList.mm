...
- (NSString*) photoImageName
{
	...
	if ([gender isEqualToString:@"F"]) {
		return [[NSBundle mainBundle]pathForResource:[NSString stringWithFormat:@"female%d",x] ofType:@"png"];
	}
	
	return [[NSBundle mainBundle]pathForResource:[NSString stringWithFormat:@"male%d",x] ofType:@"png"];
}
...
- (NSMutableArray*) allContacts
{
	...
	for (NSObject *contact in contactsOnline) {
		[allContacts addObject:contact];
	}
	for (NSObject *contact in contactsOffline) {
		[allContacts addObject:contact];
	}
	return allContacts;
}
...
- (void) addContact:(VPContact*)contact
{
	...
	if ([self contactByNumber:contact.number])
	{
		return;
	}
...
*/
	...
	[[[VPContactList sharedInstance] contactsOffline] addObject:[contact retain]];
	
	[[VPEngine sharedInstance] sendContactAdded:contact.number];
	[[VPEngine sharedInstance] postContacts:[VPContactList sharedInstance]];
	
	NSLog(@"onAdd: %@", contact.number);
...
{
	VPContact* discardedContact = nil;
	
    for (VPContact* contact in contactsOnline)
    {
        if ([contact.number isEqualToString:number])
        {
			discardedContact = contact;
			break;
        }
    }
	...
	if (discardedContact) {
		...
		[ChatMessages activityDelete:discardedContact.number];
		[contactsOnline removeObject:discardedContact];
		[[VPEngine sharedInstance] postContacts:self];
		[[NSNotificationCenter defaultCenter] postNotificationName:OnContactListCompleteNotification object:nil userInfo:nil];	
		return;
	}
	...
	if (discardedContact) {
		...
		[appDelegate activityUpdate];
		[ChatMessages activityDelete:discardedContact.number];
		[contactsOffline removeObject:discardedContact];
		[[VPEngine sharedInstance] postContacts:self];
		[[NSNotificationCenter defaultCenter] postNotificationName:OnContactListCompleteNotification object:nil userInfo:nil];	
		return;
	}
...
//            }
        ...
        {
			NSLog(@"VPMSG_QUERYONLINEACK Contact list");
			NSMutableArray *discardedItems = [NSMutableArray array];
			
			for (VPContact* contact in [contactsOnline reverseObjectEnumerator])
			{
				if (contact.status != AOL_ONLINE)
				{
					[discardedItems addObject:contact];
				}
			}
			[discardedItems reverseObjectEnumerator ];
			[contactsOffline addObjectsFromArray:discardedItems];
			[contactsOnline removeObjectsInArray:discardedItems];
			
			[discardedItems removeAllObjects];

			
			for (VPContact* contact in [contactsOffline reverseObjectEnumerator])
			{
				if (contact.status == AOL_ONLINE)
				{
					[discardedItems addObject:contact];
			...
			[contactsOnline addObjectsFromArray:discardedItems];
			[contactsOffline removeObjectsInArray:discardedItems];
...
//			contact.regCode = [fields objectAtIndex:0];
			...
			NSLog(@"Load photo: %@", contact.userName);
	...
	for (NSString *record in records) 
	{
		...
		if ([fields count] == 15) {
			if (record == [records objectAtIndex:0]) {
				...
				[[VPEngine sharedInstance] loadPhoto:profile.userName];
				...
				contact.photoUpdate = [fields objectAtIndex:13];			
				contact.flags = [[fields objectAtIndex:14] intValue];
				if ([contact.userName isEqualToString:[[VPProfile singleton] userName]]) {
					[contact release];
				} else
				{
					[contactsOffline addObject:[contact retain]];
					[self loadPhoto:contact.number lastUpdate:contact.photoUpdate];
