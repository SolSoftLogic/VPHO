...
@interface UICustomAlertView (Private)
...
- (void)show
{
	...
	[theTitle setTextColor:[UIColor titleColor]];

	UILabel *theBody = [self valueForKey:@"_bodyTextLabel"];
	[theBody setTextColor:[UIColor titleColor]];
	
	NSArray *buttonArray = [self valueForKey:@"_buttons"];
	for (UIButton *control in buttonArray) // Hack for private UIThreePartButton.h
	{
		[control setTitleColor:[UIColor titleColor] forState:UIControlStateNormal];
