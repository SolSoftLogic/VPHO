...
- (void) smsNotification:(NSUInteger)smsType number:(NSString*)number message:(NSString*)message
{
...
//					vp->GetCallRemoteName(vpcall, name);
					vp->GetCallRemoteNumber(vpcall, name);
					...
					[self smsNotification:typeSMS 
								   number:[NSString stringWithCString:name encoding:NSUTF8StringEncoding]
...
//						   aol.nums[i].online == AOL_ONLINE ? "Online" : aol.nums[i].online == AOL_LIMITED ? "Accepts only contacts" : "Unknown");
					...
					if (contact)
					{
						//							if (contact.status != aol->nums[i].online)
						{
							contact.online = aol.nums[i].online;
						}
					}
				}
				free(aol.nums);
